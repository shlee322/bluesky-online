...
public class SanghyuckTilesGenerator implements IMapTilesGenerator {
    ...
    public void generate(IMap map) {
        if(map.getMapX()<0) {
            for(int y=0; y<20; y++) {
                for(int x=0; x<20; x++) {
                    map.setTile(x, y, (byte)5);
                }
            }
        } else if(map.getMapY() == 0) {
            for(int y=11; y<20; y++) {
                for(int x=0; x<20; x++) {
                    map.setTile(x, y, (byte)(y == 11 ? 3
        ...
        for(int i=0; i<count; i++) {
            ...
            int x = r.nextInt(20);
            int y = r.nextInt(20);
            if(map.getMapY()==0 && y < 12) continue;
    ...
    private void create(IMap map, int x, int y, int id, float n, float dn) {
        ...
        if(id == 1 && (map.getMapY() > 0 || (map.getMapY() == 0 && y < 12))) //물 공중 막음
            return;

        if(x<0 || x>=20 || y<0 || y>=20) //타일 범위 초과
            return;

        map.setTile(x, y, (byte)id);
